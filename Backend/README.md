...
Allows a captain to confirm accepting a ride request.
...
Allows a captain to start a confirmed ride after OTP verification.
...
- `otp` (string, required): 6-digit OTP provided by the user

### Example Response

- `ride` (object): Updated ride object with status changed to "ongoing"

## `/rides/end-ride` Endpoint

### Description

Allows a captain to mark a ride as completed.
...
- `rideId` (string, required): The MongoDB ID of the ride to end

### Example Response

- `ride` (object): Updated ride object with status changed to "completed"
